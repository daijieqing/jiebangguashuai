...
export const RESEARCH_MODULES: ResearchModule[] = [
  {
    ...
    details: {
      feasibility: {
        technical: "基于成熟NLP技术(Apache Tika/PyPDF2)实现文档解析。核心采用 DeepSeek 等开源大模型API接口，实现文本语义理解与关键信息提取。系统采用微服务架构，可与现有系统平滑集成，技术风险可控。",
      ...
      solutionOverview: {
        architecture: "采用“数据层-分析层-应用层”三层架构设计。数据层负责多源数据接入与治理；分析层实现指标计算、关联分析和预测预警；应用层提供可视化展示与交互。",
        coreFlows: [
      ...
      demoContent: {
        ...
        demoLink: "https://test.464151859.workers.dev/", 
  ...
  },
  {
    ...
    details: {
      feasibility: {
        technical: "基于成熟NLP/OCR技术(Apache Tika)实现文档解析。核心采用 DeepSeek 等开源大模型API，具备长文本理解能力，可提取功能模块与技术指标。配合规则引擎处理数值比对，技术路径清晰，无需依赖尚未建成的省级大模型平台。",
      ...
      solutionOverview: {
        architecture: "数据接入层（对接项目管理/交易系统）→ AI解析与比对引擎（DeepSeek+规则库）→ 预警与分析层（风险评估）→ 用户交互层（决策支持）。",
        coreFlows: [
  ...
  },
  {
    ...
    details: {
      feasibility: {
        technical: "依托现有政务云平台API网关实现多源数据无缝接入。核心采用 DeepSeek 大模型智能体技术，将结构化数据转化为专业分析报告。数据清洗与整合技术成熟，技术路径清晰可行。",
      ...
      solutionOverview: {
        architecture: "采用“数据层-分析层-应用层”三层架构。数据层负责多源数据接入与治理；分析层实现指标计算、关联分析和预测预警；应用层提供可视化展示、决策支持与交互。",
        coreFlows: [
      ...
      demoContent: {
        ...
        demoLink: "https://jixiaopingjia.pages.dev/",
